...
void eval_micro_sequencer() {
  ...
  int addressing_mode_set = (cond0 & cond1 & IR11(ir))<<0;
  int read_set = ((~cond1) & cond0 & CURRENT_LATCHES.READY)<<1;
  int ben_set = (cond1 & (~cond0) & CURRENT_LATCHES.BEN)<<2;
  
  int next_j = curr_j_field + addressing_mode_set + read_set + ben_set;
  ...
  if(ird_bit == 1){
    ...
    memcpy(NEXT_LATCHES.MICROINSTRUCTION, CONTROL_STORE[next_j], sizeof(NEXT_LATCHES.MICROINSTRUCTION)); //next instruction based on j-bits 
...
//internals of the LC3 as functions
int dr_sel(int s){
    int ir = CURRENT_LATCHES.IR;
...
int GateMDR_logic(){
    ...
    mdr = (data_size == WORD) ? CURRENT_LATCHES.MDR : mdr;
    return Low16bits(mdr);
}

int LdMDR_logic(){
    int mar0 = CURRENT_LATCHES.MAR & 0x01;
    int BUS7_0 = LOWER(BUS); 
    if(mar0 == 1){// || GetDATA_SIZE(CURRENT_LATCHES.MICROINSTRUCTION) == WORD){
...
void drive_bus() {
   ...
   */       
    ...
    if(GetGATE_ALU(CURRENT_LATCHES.MICROINSTRUCTION) == YES){BUS = GATE_ALU_VAL;}
    if(GetGATE_MARMUX(CURRENT_LATCHES.MICROINSTRUCTION) == YES){BUS = GATE_MAR_VAL;}
    if(GetGATE_MDR(CURRENT_LATCHES.MICROINSTRUCTION) == YES){BUS = GATE_MDR_VAL;}
    if(GetGATE_PC(CURRENT_LATCHES.MICROINSTRUCTION) == YES){BUS = GATE_PC_VAL;}
    if(GetGATE_SHF(CURRENT_LATCHES.MICROINSTRUCTION) == YES){BUS = GATE_SHF_VAL;}
...
void latch_datapath_values() {
   ...
   */       
    ...
    if(GetLD_CC(CURRENT_LATCHES.MICROINSTRUCTION) == LOAD){
        int new_cc = nzp_logic(SEXT(BUS,16));
        NEXT_LATCHES.P = 0;
        NEXT_LATCHES.Z = 0;
        NEXT_LATCHES.N = 0;
        switch ((new_cc))
        {
            case -1: NEXT_LATCHES.N = 1;break;
            case 0: NEXT_LATCHES.Z = 1; break;
            case 1: NEXT_LATCHES.P = 1;break;
            case 7: printf("something went terribly wrong");
            default: break;
        }
    }
    ...
    if(GetLD_MDR(CURRENT_LATCHES.MICROINSTRUCTION) == LOAD){
        ...
        NEXT_LATCHES.MDR = mdr_mux(mem_output, bus_val);
    }
    ...
    if(GetLD_PC(CURRENT_LATCHES.MICROINSTRUCTION) == LOAD){
        ...
        int lshf1_result = LSHF1_block(addr2_result);
        int adder = ADDER(addr1_result, lshf1_result );
        NEXT_LATCHES.PC = pc_mux(GetPCMUX(CURRENT_LATCHES.MICROINSTRUCTION), pc+2, BUS, adder);
    }
